...
the launcher is not on `PATH`, the skill-local CLI under this skill's own
directory (`~/.claude/skills/council` under Claude Code, `~/.codex/skills/council`
under Codex). It shells out to `claude -p` and `codex exec` and targets the
...
Run it from the project you want council to work on. It installs into both
`~/.claude/skills/council` and `~/.codex/skills/council` via agent-kit's
...
1. Clarify. If scope, constraints, definition of done, or repo area are
   ambiguous, ask 2-3 targeted questions (use `AskUserQuestion` where
   available); otherwise skip. Write the result to `brief.md`.

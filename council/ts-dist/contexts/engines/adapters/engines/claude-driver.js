...
export class ClaudeEngineDriver {
    run(request, ports) {
        const command = {
            ...commandParts('claude', [
                '-p',
        ...
        const state = new ClaudeStreamJsonState();
        return createSpawnedEngineStream({
            ...
            parseStdoutLine: (line) => state.parseStdoutLine(line),
            readResult: () => state.readResult(),
        });
    }
}
export const CLAUDE_ENGINE_DRIVER = new ClaudeEngineDriver();
export function runClaudeEngine(request, ports) {
    return CLAUDE_ENGINE_DRIVER.run(request, ports);
}
class ClaudeStreamJsonState {
    lastResult;
    ...
    readResult() {
        ...
        return resultExtraction(resultText(lastResult.result), readNumber(lastResult.total_cost_usd) ?? readNumber(lastResult.cost_usd), lastResult);
    }
}

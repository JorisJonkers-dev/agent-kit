import { omitUndefined, optionalBoolean, optionalNumber, optionalObject, optionalString, optionalStringArray, optionalStringRecord, optionalStringRecordTable, } from './config-value-objects.js';
import { optionalCodexEffort, optionalIntensity, ROLE_KEYS } from './presets.js';
export function normalizeCouncilConfig(data) {
    return omitUndefined({
        ...
        watchdog: optionalObject(data.watchdog, (watchdog) => omitUndefined({
        ...
        design: optionalObject(data.design, (design) => omitUndefined({
            lenses: optionalStringArray(design.lenses),
            rounds: optionalNumber(design.rounds),
            stages: optionalStringRecordTable(design.stages, (stage) => omitUndefined({
                engine: optionalString(stage.engine),
                effort: optionalString(stage.effort),
            })),
        })),
        review: optionalObject(data.review, (review) => omitUndefined({
        ...
        github: optionalObject(data.github, (github) => omitUndefined({
            enabled: optionalBoolean(github.enabled),
            assignee: optionalString(github.assignee),
        })),
        engines: optionalStringRecordTable(data.engines, (engine) => omitUndefined({
        ...
        triage: optionalObject(data.triage, (triage) => omitUndefined({
            matrix_overrides: optionalStringRecord(triage.matrix_overrides),
        })),
        context: optionalObject(data.context, (context) => omitUndefined({
            pack_stale_after_s: optionalNumber(context.pack_stale_after_s),
        })),
        model_matrix: optionalObject(data.model_matrix, (modelMatrix) => omitUndefined({
            roles: optionalRoleRecord(modelMatrix.roles),
            intensity: optionalStringRecordTable(modelMatrix.intensity, (preset) => omitUndefined({

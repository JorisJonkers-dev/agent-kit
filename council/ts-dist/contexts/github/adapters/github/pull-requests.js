...
import { hasPullRequestShape, isDryRun, logFailure, optionalTrimmed, parseJsonObject, } from './helpers.js';
export async function detectDefaultBranch(context, cwd) {
    try {
...
export async function createPullRequest(context, request, options = {}) {
    try {
        ...
        return parseJsonObject(await context.client.ghJson(request.cwd, args), hasPullRequestShape);
    }
    catch (error) {
...
export async function viewPullRequest(context, cwd, number) {
    try {
        return parseJsonObject(await context.client.ghJson(cwd, ['pr', 'view', String(number), '--json', 'number,url']), hasPullRequestShape);
    }
    catch (error) {

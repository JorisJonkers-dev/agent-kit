import { buildStatusLabelTransition, findIssueByTaskMarker, issueStateAfterMirror, renderTaskIssueBody, selectBestFitLabels, } from '../../../github/index.js';
import { addJoinedOption, existingLabelName, isDryRun, logFailure, parseJsonArray, uniqueStrings, } from './helpers.js';
import { listVerifiedLabels } from './labels.js';
import { ensureMilestone } from './milestones.js';
export async function listIssues(context, cwd) {
    try {
...
export async function mirrorTaskIssue(context, cwd, request, options = {}) {
    try {
...
async function createTaskIssue(context, cwd, request, body, labels, milestone) {
...
async function updateTaskIssue(context, cwd, issue, request, body, labels, milestone) {
    const args = [
        'issue',
        'edit',
        String(issue.number),
...
function hasIssueShape(candidate) {
    return (candidate !== null &&
        typeof candidate === 'object' &&
        typeof candidate.number === 'number' &&
        typeof candidate.title === 'string');
}
function issueNumberFromOutput(output) {

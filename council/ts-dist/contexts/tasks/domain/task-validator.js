import { comparableKey, formatPythonList, isJsonArray, isJsonRecord, pythonRepr, stringifyForDisplay, } from './task-json.js';
const REQUIRED_VALIDATE_FIELDS = ['id', 'objective', 'depends_on', 'paths', 'model', 'verify'];
export function validateTasks(tasks, options = {}) {
    ...
    for (const task of tasks) {
        ...
        if (!stringifyForDisplay(task.verify).trim()) {
            options.onWarning?.(`warning: task ${stringifyForDisplay(task.id)} has no verify command - its result is unchecked except by the adversarial verifier`);
        }
    }
    assertTaskDag(tasks);
}
function assertTaskDag(tasks) {
    ...
    for (const task of tasks) {
        ...
        for (const dep of task.depends_on) {
            ...
            if (!ids.has(depKey)) {
                throw new Error(`task ${pythonRepr(stringifyForDisplay(task.id))} depends on unknown task ${pythonRepr(stringifyForDisplay(dep))}`);
            }
        }
    }

import { isJsonRecord, jsonErrorMessage, pythonRepr, stableJsonStringify, stringifyForDisplay, } from './task-json.js';
import { validateTasks } from './task-validator.js';
const TASK_BLOCK_RE = /^## (?<headerId>[^\n:]+)(?::[^\n]*)?\n<!-- council-task-id: (?<markerId>[^>]+) -->\n```json\n(?<body>.*?)\n```/gms;
export function renderTasksMd(tasks, specRef) {
    ...
    for (const task of tasks) {
        ...
        lines.push(`## ${id}: ${taskTitle}`, `<!-- council-task-id: ${id} -->`, '```json', stableJsonStringify(task), '```', '');
    }
    return `${lines.join('\n').trimEnd()}\n`;
}
...
export function assertTasksBijection(tasks, tasksMdText) {

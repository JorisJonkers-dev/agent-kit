...
const DESIGN_LEDGER_FILE = 'design-ledger.json'
...
export class FsRunStoreAdapter implements RunStorePort {
  ...
  constructor(root: string, options: FsRunStoreOptions = {}) {
    ...
    this.tempId =
      options.tempId ??
      (() =>
        `tmp-${String(this.clock.now().getTime())}-${String(process.pid)}-${String(++this.tempCounter)}`)
  }
  ...
  async writeState(runId: string, state: RunState): Promise<void> {
    ...
    await this.executeAtomicJsonPlan(planStateWrite(runId, state, this.tempId()))
  }
  ...
  async writeTasks(runId: string, tasks: readonly Task[]): Promise<void> {
    assertTasks(tasks)
    await this.executeAtomicJsonPlan(planTasksWrite(runId, tasks, this.tempId()))
  }
  ...
  async writeStory(runId: string, story: Story): Promise<void> {
    assertStory(story)
    await this.atomicWriteJson(this.runFile(runId, STORY_FILE), story)
  }
  ...
  async writeWorkerResult(runId: string, taskId: string, result: WorkerResult): Promise<void> {
    ...
    await this.atomicWriteJson(this.runFile(runId, WORKERS_DIR, taskId, RESULT_FILE), result)
  }

  private async appendEvent(runId: string, event: RunStoreEvent): Promise<void> {
  ...
  private async executeAtomicJsonPlan(
    plan: ReturnType<typeof planStateWrite> | ReturnType<typeof planTasksWrite>,
  ): Promise<void> {
    await this.atomicWriteBytes(join(this.root, plan.finalPath), join(this.root, plan.tempPath), plan.bytes)
  }

  private async atomicWriteJson(path: string, value: unknown): Promise<void> {
    const id = this.tempId()
    assertPathSegment('tempId', id)
    await this.atomicWriteBytes(
      path,
  ...
  private async atomicWriteBytes(finalPath: string, tempPath: string, bytes: string): Promise<void> {
...
type RunStoreEvent =
  | ReturnType<typeof reviewVerdictEvent>
...
function assertRunState(value: unknown): RunState {
  const record = assertRecord(value, 'state')
...
function assertTasks(value: unknown): readonly Task[] {
  validateTasks(value)
  assertTasksJsonSchema(value)
  return value as unknown as readonly Task[]
}

function assertStory(value: unknown): Story {
...
function assertDesignLedger(value: unknown): DesignLedger {
...
function assertReviewVerdict(value: unknown): ReviewVerdict {
...
function assertRoutingVerdict(value: unknown): RoutingVerdict {
...
function assertAmendment(value: unknown): Amendment {
...
function assertWorkerResult(value: unknown, taskId?: string): WorkerResult {
...
function assertLegacyReport(value: unknown): LegacyRunReport {
...
function assertRunStoreEvent(value: unknown): RunStoreEvent {
...
function copyOptionalString(from: JsonRecord, to: Record<string, unknown>, field: string): void {
  if (typeof from[field] === 'string') to[field] = from[field]
}

function copyOptionalInteger(from: JsonRecord, to: Record<string, unknown>, field: string): void {

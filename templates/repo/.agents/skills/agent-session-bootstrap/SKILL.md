...
1. Locate the active user and project config layers:
   - Claude: `~/.claude/settings.json`, `~/.claude/CLAUDE.md`, project
     `CLAUDE.md`, project `.claude/settings.json`, and `~/.claude/skills`.
...
4. Add or update global memory files so future sessions know to consult and
   update the KB without user reminders.
5. Validate with at least one `tools/list` or `knowledge.recall` MCP call.

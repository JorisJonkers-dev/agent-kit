---
name: council-design
description: Produce or refine a council-ready design, consolidated plan, and parallel task DAG. Use when Codex needs to turn a clear brief into bounded chunks with dependencies, validation, and model assignments before execution.
---
...
- Primary command shape: `node ~/.codex/skills/council/council.mjs design ...`.
- Use `council-grill` to challenge the design before `council-run`.
- Use `claude-worker` for an independent architecture or product pass.
